...
#'
#' Selects a point along the regularization path of a fitted grpreg object according to the AIC,
#' BIC, or GCV criteria.
#'
...
#' @param criterion The criterion by which to select the regularization parameter. One of `"AIC"`,
#'   `"BIC"`, `"GCV"`, `"AICc"`, or `"EBIC"`; default is `"BIC"`.
...
#' @param smooth Applies a smoother to the information criteria before selecting the optimal value.
...
#'   \item{IC}{
#'     A vector of the calculated model selection criteria for each point on the regularization
#'     path.
#'   }
#' }
#'
...
select.grpreg <- function(
  ...
  return(list(beta = obj$beta[, i], lambda = obj$lambda[i], df = df[i], IC = IC))
}
