...
#'
#' Plots a spline curve for a single variable using a `grpreg` or `cv.grpreg` object for which an
#' additive model was fit.
#'
#' `plot_spline()` takes a model fit using both the [grpreg()] and [expand_spline()] functions and
#' plots a spline curve for a given variable.
#'
...
#' @param lambda Values of the regularization parameter `lambda` which will be used for the plot. If
#'   a vector is passed, a curve will be drawn for each value of lambda (numeric vector; if a
#'   `cv.grpreg` object is passed, the `lambda` value minimizing cross-validation error will be used
#'   as a default; otherwise, there is no default value)
#' @param which Index of penalty parameter `lambda` which will be used for the plot. If both
#'   `lambda` and `which` are specified, `lambda` takes precedence (integer vector).
#' @param partial If `TRUE`, a scatter plot of the partial residuals is superimposed on the curve
#'   (logical; default = `FALSE`). If multiple lambdas are specified, the largest value is used to
#'   calculate the residuals.
#' @param type Type of plot to be produced (default = `"contrast"`). The following options are
#'   supported:
#'   - If `"conditional"`, the plot returned shows the value of the variable on the x-axis and the
#'     change in linear predictor on the y-axis, holding all other variables constant at their mean
#'     value.
#'   - If `"contrast"`, the plot returned shows the effect on the linear predictor by moving the x
#'     variable away from its mean.
...
#' @param ... Further arguments to be passed to `plot()`. Note that these arguments also control the
...
#' plot_spline(fit, "V02",
#'   lambda = 0.03, partial = TRUE, lwd = 6, col = "yellow",
...
plot_spline <- function(
  ...
  if (inherits(fit, "cv.grpreg")) {
    if (missing(lambda) & missing(which)) {
      lambda <- fit$lambda.min
    }
    fit <- fit$fit
  }
  if (!inherits(fit, "expanded")) {
    stop("Model must have been fit using a expand_spline object", call. = FALSE)
  }
  ...
  if (missing(lambda)) {
    ...
    if (!all(which %in% 1:length(fit$lambda))) {
      stop("'which' must be an integer between 1 and length(fit$lambda)", call. = FALSE)
    }
  ...
  if (partial == TRUE && length(lambda) > 1 && warnings == TRUE) {
