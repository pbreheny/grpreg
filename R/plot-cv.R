...
#'
#' Error bars representing approximate +/- 1 SE (68\% confidence intervals) are plotted along with
#' the estimates at value of `lambda`. For `rsq` and `snr`, these confidence intervals are quite
#' crude, especially near zero, and will hopefully be improved upon in later versions of `grpreg`.
#'
#' @param x A `cv.grpreg` object.
#' @param log.l Should horizontal axis be on the log scale? Default is TRUE.
#' @param type What to plot on the vertical axis. `cve` plots the cross-validation error (deviance);
#'   `rsq` plots an estimate of the fraction of the deviance explained by the model (R-squared);
#'   `snr` plots an estimate of the signal-to-noise ratio; `scale` plots, for `family="gaussian"`,
#'   an estimate of the scale parameter (standard deviation); `pred` plots, for `family="binomial"`,
#'   the estimated prediction error; `all` produces all of the above.
#' @param selected If `TRUE` (the default), places an axis on top of the plot denoting the number of
#'   groups in the model (i.e., that contain a nonzero regression coefficient) at that value of
#'   `lambda`.
...
plot.cv.grpreg <- function(
  ...
  if (type == "cve") {
    ...
    if (type == "rsq") {
      ...
      if (max(c(y, L, U)) == 1e6) {
        warning("Signal-to-noise ratio is infinite")
      }
