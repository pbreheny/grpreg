grpreg <- function(X, y, group=1:ncol(X), inner=c("L2","L1","MCP"), outer=c("L1","MCP","SCAD"), penalty, family=c("gaussian","binomial"), nlambda=100, lambda, lambda.min={if (n>p) .001 else .05}, alpha=1, eps=.005, max.iter=1000, dfmax=p, gamma, group.multiplier=rep(1,J), verbose=FALSE, warn.conv=TRUE, ...)
{
  ## Check for errors
