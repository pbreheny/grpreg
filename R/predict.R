...
#'
#' Similar to other predict methods, this function returns predictions from a fitted `grpreg`
#' object.
#'
#' `coef` and `predict` methods are provided for `cv.grpreg` options as a convenience. They simply
#' call `coef.grpreg` and `predict.grpreg` with `lambda` set to the value that minimizes the
#' cross-validation error.
#'
#' @param object Fitted `grpreg` or `cv.grpreg` model object.
...
#'   - `groups` returns a list containing the indices for the groups with at least one nonzero
#'     coefficient
#'   - `nvars` returns the number of nonzero coefficients at each value of `lambda`
#'   - `ngroups` returns the number of groups with at least one nonzero coefficient
#'   ` `norm` returns the L2 norm of the coefficients in each group
#' @param drop By default, if a single value of `lambda` is supplied, a vector of coefficients is
#'   returned. Set `drop=FALSE` if you wish to have `coef` always return a matrix (see
#'   [base::drop()]).
#' @param ... Not used.
#'
#' @returns
  ...
  if (d == 2) {
    ...
    if (type == "norm") {
      return(drop(apply(beta, 2, function(x) {
        tapply(x, object$group, function(x) {
          sqrt(sum(x^2))
        })
      })))
    }
    if (missing(X) | is.null(X)) {
      stop("Must supply X", call. = FALSE)
    }
    if (inherits(object, "expanded")) {
      X <- predict_spline(object, X)
    }
    eta <- sweep(X %*% beta, 2, alpha, "+")
    if (object$family == "gaussian" & type == "class") {
      stop("type='class' is not applicable for family='gaussian'", call. = FALSE)
    }
    ...
    if (type == "vars") {
      stop("Predicting type 'vars' not implemented with multivariate outcomes", call. = FALSE)
    }
    if (type == "groups") {
      return(drop(apply(beta, 3, function(x) which(apply(x != 0, 2, any)))))
    }
    if (type == "norm") {
    ...
    eta <- apply(beta, 1, function(b) {
      X %*% b
    })
    eta <- array(
      eta,
      dim = c(nrow(X), dim(beta)[1], dim(beta)[3]),
      dimnames = list(NULL, dimnames(beta)[[1]], dimnames(beta)[[3]])
    )
