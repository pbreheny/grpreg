...
#'
#' @param x Fitted `"grpreg"` model.
#' @param alpha Controls alpha-blending. Default is alpha=1.
#' @param legend.loc Where should the legend go? If left unspecified, no legend is drawn. See
#'   [graphics::legend()] for details.
#' @param label If TRUE, annotates the plot with text labels in the right margin describing which
#'   variable/group the corresponding line belongs to.
#' @param log.l Should horizontal axis be on the log scale? Default is FALSE.
#' @param norm If `TRUE`, plot the norm of each group, rather than the individual coefficients.
#' @param ... Other graphical parameters to `plot`, `matlines`, or `legend`
#'
...
#' labs <- c(
#'   "Mother's Age", "# Phys. visits", "Hypertension", "Mother's weight",
