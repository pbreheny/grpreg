...
- New: plot_spline() now has “add” option so that splines can be added
  to existing plot
- Fixed: Loss/deviance now used consistently throughout; see
  [\#52](https://github.com/pbreheny/grpreg/issues/52)
...
- Fixed: No more error if response is constant; see
  [\#46](https://github.com/pbreheny/grpreg/issues/46)
...
- New: Suite of tools for additive modeling, most notably
  expand_spline() and plot_spline() (thank you to Ryan Kurth for her
...
- Change: ‘Lung’ and ‘Birthwt’ data sets now use factor representation
  of group, as character vectors are inherently ambiguous with respect
  to order
- Change: max.iter now based on total number of iterations for entire
  path
- Internal: ‘X’, ‘group’, and ‘group.multiplier’ now bundled together in
  an object called ‘XG’ to enforce agreement at all times
...
- Fixed: The binding of X and G fixes several potential bugs, including
  Issue [\#12](https://github.com/pbreheny/grpreg/issues/12) (GitHub)
...
- New: methods for survival analysis (Cox modeling): grpsurv,
...
- Internal: Greatly expanded suite of tests; various bugs identified and
  fixed as a result
...
- New: More flexible interface through the ‘group’ argument; groups may
  now be out of order, and may be named rather than only consecutive
  integers
- New: ‘X’ can now be a matrix of integers (previously this would result
  in the passing of an incompatible storage type to C)
...
- New: grpreg and cv.grpreg now allow matrix y to facilitation group
  ...
  learning. This is something of a ‘beta’ release at this point, and
  will be developed and refined further in future releases.
