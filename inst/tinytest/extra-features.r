...
expect_equivalent(
  apply(
    grpreg:::deviance_grpreg(y, predict(fit, X = X, type = "response"), family = "gaussian"),
    2,
    sum
  ),
  fit$deviance
...
expect_equivalent(
  apply(
    grpreg:::deviance_grpreg(yy, predict(fit, X = X, type = "response"), family = "binomial"),
    2,
    sum
  ),
  fit$deviance,
...
expect_equivalent(
  apply(
    grpreg:::deviance_grpreg(yy, predict(fit, X = X, type = "response"), family = "poisson"),
    2,
    sum
  ),
  fit$deviance,
...
plot(
  fit <- grpreg(X, y, group, penalty = "grLasso", lambda.min = 0, group.multiplier = gm),
  main = fit$penalty
)
plot(
  fit <- grpreg(X, y, group, penalty = "grMCP", lambda.min = 0, group.multiplier = gm),
  main = fit$penalty
)
plot(
  fit <- grpreg(X, y, group, penalty = "grSCAD", lambda.min = 0, group.multiplier = gm),
  main = fit$penalty
)
